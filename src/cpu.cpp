...
namespace SysInfo::CPU {
    ...
    const CPUInfo::Architecture& CPUInfo::GetArchitecture() const noexcept { return m_architecture; }
    const CPUInfo::ProcType& CPUInfo::GetProcessorType() const noexcept { return m_procType; }
    const std::string& CPUInfo::GetProcessorID() const noexcept { return m_procID; }
    const std::uint16_t& CPUInfo::GetCores() const noexcept { return m_cores; }
    const std::uint16_t& CPUInfo::GetThreads() const noexcept { return m_threads; }
    const std::uint16_t& CPUInfo::GetProcessorRevision() const noexcept { return m_procRevision; }
    const std::uint32_t& CPUInfo::GetClockSpeed() const noexcept { return m_clockSpeed; }
    const std::string& CPUInfo::GetSocket() const noexcept { return m_socket; }
    const std::string& CPUInfo::GetProcessorVendorName() const noexcept { return m_vendor; }
    ...
    std::string CPUInfo::GetProcessorTypeString() const noexcept {
        return CPUInfo::ProcessorTypeToString(GetProcessorType());
    }

    CPUInfo::Architecture CPUInfo::ValueToArchitecture(std::uint16_t value) noexcept {
        switch (value) {
    ...
    std::string CPUInfo::ArchitectureToString(const Architecture& arch) noexcept {
        switch (arch) {
    ...
    std::string CPUInfo::ProcessorTypeToString(const ProcType& type) noexcept {
        switch (type) {
            case ProcType::OEM:
                return "OEM";
            case ProcType::IOP:
